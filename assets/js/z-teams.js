...
matches.on("change", async (matchList) => {
	...
	if (loadedMatchIndex !== undefined) {
		...
		loadedMatch = cachedMatchList.filter(
			(mtch) => mtch.matchId == loadedMatchIndex
		)[0];
